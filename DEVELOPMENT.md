...
- **Trigger name**: `trg_maintenance_{descriptive_name}`
- **Function name**: `trg_maintenance_{descriptive_name}_fn`

Where `maintenance` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_maintenance_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_maintenance_audit_log"
AFTER INSERT OR UPDATE ON "module_maintenance_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_maintenance_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_maintenance_*` triggers when the plugin
